...
module.exports = {
   name: 'setchatbot',
    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.permission.has(Permissions.FLAGS.MANAGE_GUILD)) return message.channel.send({embed: {
...
if (!args[0]) {
  ...
  if (message.guild.channels.cache.has(b)) {
    return message.channel.send({ content: `**${emote.verified} ChatBot Channel Set In This Server Is \`${channelName.name}\`!**`
    })
  } else
    ...
    try {
        ...
        return message.channel.send({ content: `**${emote.error} Error - Missing Permissions Or Channel Is Not A Text Channel!**`})
    }
    }
}