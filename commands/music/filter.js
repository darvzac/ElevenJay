...
module.exports = {
   name: "filter",
    ...
    run: async (client, message, args) => {
		...
		if (!args[0]) return message.channel.send({ embeds: [embed4] });
		
		const filterToUpdate = Object.keys(client.filters).find((x) => x.toLowerCase() === args[0].toLowerCase());
		
		if (!filterToUpdate) return message.channel.send({ embeds: [embed5] });
		...
		if (filtersUpdated[filterToUpdate]) message.channel.send({ embeds: [embed6] });
		else message.channel.send({ embeds: [embed7] });
	}
